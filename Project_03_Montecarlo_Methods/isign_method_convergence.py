...
Econv=False
Mconv=False
while(((Econv == False) or (Mconv == False)) and (len(E) <= msrmnt)): #or (len(E) <= msrmnt)): 
        #call MC calculation
        ...
        if t%10 == 0:
                ...
                print("Econv:",Econv,"Mconv:",Mconv, "Len(E)", len(E))
        if (len(E)>=200):
                LastE = E[-200:]                    # take the last 100 energy values
                LastM = M[-200:]                    # take the last 100 magnetization values
                DifE = max(LastE)-min(LastE)
                #print(DifE,"< que",3/temp)
                DifM = max(LastM)-min(LastM)
                #print(DifM,"< que", 0.1)
                if (DifE < 2*temp):
                        Econv = True
                if (DifM < 0.075):
...
print("The simulation converged before the maxium", msrmnt, "steps you set")
